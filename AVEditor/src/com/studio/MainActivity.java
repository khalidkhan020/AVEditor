...
installFfmpeg();
	...
	/*findViewById(R.id.youtu).setOnClickListener(new OnClickListener()
	{
		...
		{
			final com.youtubekk.FfmpegJob job = new com.youtubekk.FfmpegJob(mYoutubeInstallPath		);
			...
			Mysynmerge = new AsyncTask<Void, Void, Void>() {
				...
				protected Void doInBackground(Void... arg0) {
					try {
						ProcessRunnable r=job.create5();
						r.setProcessListener(new  ProcessListener()
						{
				...
				@Override
				protected void onPostExecute(Void resulht) {
					try {
						progressDialog.dismiss();
					} catch (Exception e) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 2) {
			if (data != null) {
				String arr = data.getExtras().getString("array");
				String start = data.getExtras().getString("start");
				String duration = data.getExtras().getString("duration");
				String framerate = data.getExtras().getString("framerate");
				String filename = data.getExtras().getString("filename");
				String picformat = data.getExtras().getString("picformat");
				String file_path = data.getExtras().getString("videofilepath");
